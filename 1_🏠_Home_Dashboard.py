...
# Load Data
# ===================
def load_order_confirmation_data():
...
customer_gp["invoiced_gross_profit_usd"] = customer_gp["invoiced_gross_profit_usd"].astype(float)
customer_gp["invoiced_amount_usd"] = customer_gp["invoiced_amount_usd"].astype(float)
customer_gp["gp_percent"] = (customer_gp["invoiced_gross_profit_usd"] / customer_gp["invoiced_amount_usd"]) * 100

# Sắp xếp theo GP
customer_gp = customer_gp.sort_values(by="invoiced_gross_profit_usd", ascending=False)

# Tính % cộng dồn
customer_gp["cumulative_percent"] = customer_gp["invoiced_gross_profit_usd"].cumsum() / customer_gp["invoiced_gross_profit_usd"].sum() * 100
